...
if ($result && $result->num_rows > 0) {
    while($nf = $result->fetch_assoc()) {
        $nf_id = (int)$nf['id'];
        $fertilizantes = [];
        $res_fert = $conn->query("SELECT nome_produto, quantidade, unidade FROM fertilizantes_aceitos WHERE nf_id = $nf_id");
...
<style>
    ...
    nav a { margin-right: 10px; color: #333; text-decoration: none; font-weight: bold; }
...
<header>
    ...
    <nav>
        ...
        <a href="pendente.php">NF's Pendentes</a>
    </nav>
</header>
                    ...
                    <tr id="expand_<?php echo $nf['id']; ?>" class="detalhes-nf" style="display:none;">
                        <td colspan="9">
                            <div class="detalhes-conteudo">
                                ...
                                <strong>CPF do responsável:</strong> <?php echo htmlspecialchars($nf['cpf_responsavel']); ?><br><br>

                                <strong>Fertilizantes nesta NF:</strong><br>
                                <?php if (!empty($nf['fertilizantes'])): ?>
                                    <ul>
                                    <?php foreach($nf['fertilizantes'] as $f): ?>
                                        <li><?php echo htmlspecialchars($f['nome_produto']); ?> — 
